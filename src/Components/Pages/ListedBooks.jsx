...
const [tabIndex, setTabIndex] =useState(0)
  ...
  const handelBookFilter = (filter) => {
    if (filter === "rating") {
      const RatingBookFilter = addToBooks.filter(
        (bookCard) => bookCard.rating === "rating"
      );
      setDisplayBooks(RatingBookFilter);
    } else if (filter === "totalPages") {
      const TotalPageBookFilter = addToBooks.filter(
        (bookCard) => bookCard.totalPages === "totalPages"
      );
      setDisplayBooks(TotalPageBookFilter);
    } else if (filter === "yearOfPublishing") {
      const publishBookFilter = addToBooks.filter(
        (bookCard) => bookCard.totalPages === "yearOfPublishing"
      );
  ...
  return (
    ...
    className={`flex items-center flex-shrink-0 px-5 py-3 space-x-2 ${tabIndex === 0 ? 'border border-b-0': 'border-b'} rounded-t-lg border-gray-400 text-[#13131380]`}>
